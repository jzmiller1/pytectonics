...
class Crust(GeoCoordinate):
    ...
    def isDetaching(self):
        """Returns whether subducting crust should be detaching from overriding
        crust Detachment occurs after travelling a given distance under
        ...
        * there is no known model for subduction angles that does not 
        ...
        if not self._firstSubductedBy:
            ...
            distanceSubductedAngular = self.getArcDistance(
                self._firstSubductedBy)
            distanceSubducted = self.world.radiansToDistance(
                distanceSubductedAngular)
