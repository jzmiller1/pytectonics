...
class Plate(GeoCoordinate):
    ...
    def _getCollidable(self):
        ...
        if not self._collidable:
            self._collidable = set([crust for crust in self.grid if
                                    any(self.getCollidableNeighborIds(crust))])
        ...
        while unprocessed:
            ...
            while neighbors:
                neighbors = [neighbor for neighbor in
                             self.grid.getNeighbors(neighbors,
                                                    getIsNeighbors=getIsNeighbors)
    ...
    def getAngularSpeed(self, timestep=1):
        ...
        smallCircleRadius = moment(self.cartesian,
                                   self.eulerPole) * self.world.radius
    ...
    def requestDock(self, top, bottom):
        if not top.plate.isDockRequested(
                bottom) and not bottom.plate.isDockRequested(top):
            # merging plates together
            bottomP = bottom.plate
            bottomGroup = bottomP.getGroup(bottom,
                                           lambda crust1, crust2: crust2.isContinent())
            topP = top.plate
            topGroup = topP.getGroup(top,
                                     lambda crust1, crust2: crust2.isContinent())
            ...
            momentum = dockedTo.getMomentum(larger) + docking.getMomentum(
                smaller)
            ...
            if self.grid[id]:
                nearest = self.grid.getNearest(crust.cartesian,
                                               points=[GeoCoordinate(
                                                   cartesian=self.grid.getCartesian(
                                                       neighborId),
                                                   id=neighborId)
                                                   for neighborId in
                                                   self.grid.getCellNeighborIds(
                                                       id)])
                id = nearest.id
